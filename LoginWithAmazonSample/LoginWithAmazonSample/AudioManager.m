...
@property (copy, nonatomic) void (^recordCompletedHandler)(NSData *data);
...
- (void)recordStartWithProcess:(void (^)(float peakPower))processHandler
    ...
    dispatch_async(queue, ^{
        if (!self.audioRecorder.isRecording) {
            ...
            self.recordCompletedHandler = completedHandler;
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag {
    if (flag) {
        if (self.recordCompletedHandler) {
            NSData *data = [[NSData alloc] initWithContentsOfURL:recorder.url];
           // dispatch_async(dispatch_get_main_queue(), ^{
                self.recordCompletedHandler(data);
          //  });
        }
    } else {
